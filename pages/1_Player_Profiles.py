...
with col_photo:
    ...
    if foto:
        ...
        st.markdown("""
        <div style="width:200px;height:200px;background:rgba(255,255,255,0.05);
    ...
    <div style="padding:20px 0">
        ...
        <div style="margin-top:12px">
            <span style="background:{estado_color}22;color:{estado_color};border:1px solid {estado_color};
                         border-radius:8px;padding:4px 14px;font-size:0.85rem;font-weight:700">{estado_icon}</span>
        </div>
...
with c3: st.metric("🏆 Win% GS", fmt_pct(player.get("winpct_grand_slams_52w")), player.get("record_grand_slams_52w",""))
...
with col1:
    ...
    for label, col in [("1st Serve %","1er_saque_pct_2025"),
                        ("1st Serve Won %","1er_saque_ganado_pct_2025"),
                        ("2nd Serve Won %","2do_saque_ganado_pct_2025"),
                        ("Service Games Won %","juegos_saque_g_pct_2025"),
...
with col2:
    ...
    for label, col in [("1st Serve %","1er_saque_pct_2026"),
                        ("1st Serve Won %","1er_saque_ganado_pct_2026"),
                        ("2nd Serve Won %","2do_saque_ganado_pct_2026"),
                        ("Service Games Won %","juegos_saque_g_pct_2026"),
...
with col1:
    st.markdown("**Season 2025**")
    for label, col in [("1st Serve Return %","resto_1er_pct_2025"),
                        ("2nd Serve Return %","resto_2do_pct_2025"),
                        ("BP Converted %","bp_convertidos_pct_2025"),
                        ("Return Games Won %","juegos_resto_g_pct_2025")]:
        val = player.get(col)
        if pd.notna(val):
            st.markdown(f'<div class="stat-row"><span class="stat-label">{label}</span><span class="stat-value">{val:.1f}%</span></div>', unsafe_allow_html=True)
...
with col2:
    st.markdown("**Season 2026**")
    for label, col in [("1st Serve Return %","resto_1er_pct_2026"),
                        ("2nd Serve Return %","resto_2do_pct_2026"),
                        ("BP Converted %","bp_convertidos_pct_2026"),
                        ("Return Games Won %","juegos_resto_g_pct_2026")]:
        val = player.get(col)
        if pd.notna(val):
            st.markdown(f'<div class="stat-row"><span class="stat-label">{label}</span><span class="stat-value">{val:.1f}%</span></div>', unsafe_allow_html=True)
