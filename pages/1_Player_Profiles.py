...
fotos = {
    ...
    "Holger Rune":                  "https://www.atptour.com/-/media/alias/player-headshot/RH16",
    ...
    "Adrian Mannarino":             "https://www.atptour.com/-/media/alias/player-headshot/ME82",
    ...
    "Joao Fonseca":                 "https://www.atptour.com/-/media/alias/player-headshot/F0FS",
    "Luciano Darderi":              "https://www.atptour.com/-/media/alias/player-headshot/D0F6",
    ...
    "Alexei Popyrin":               "https://www.atptour.com/-/media/alias/player-headshot/P09Z",
    ...
    "Tallon Griekspoor":            "https://www.atptour.com/-/media/alias/player-headshot/GJ37",
    ...
    "Fabian Marozsan":              "https://www.atptour.com/-/media/alias/player-headshot/M0CI",
    ...
    "Marton Fucsovics":             "https://www.atptour.com/-/media/alias/player-headshot/F724",
    "Corentin Moutet":              "https://www.atptour.com/-/media/alias/player-headshot/MB34",
}
...
points = int(player["atp_points_current"]) if pd.notna(player.get("atp_points_current")) else None
...
if estado == "LESIONADO":
    estado_color = "#ff4444"; estado_icon = "INJURED"
elif estado == "DUDA":
...
nota_html  = f'<p style="color:rgba(255,255,255,0.4);font-size:0.8rem;margin-top:8px">{nota}</p>' if nota and str(nota) != 'nan' else ''
...
with col1:
    ...
    for label, col in [("1st Serve %","1er_saque_pct_2025"),("1st Serve Won %","1er_saque_ganado_pct_2025"),
                        ("2nd Serve Won %","2do_saque_ganado_pct_2025"),("Service Games Won %","juegos_saque_g_pct_2025"),
                        ("BP Saved %","bp_salvados_pct_2025")]:
        val = player.get(col)
        if pd.notna(val):
...
with col2:
    ...
    for label, col in [("1st Serve %","1er_saque_pct_2026"),("1st Serve Won %","1er_saque_ganado_pct_2026"),
                        ("2nd Serve Won %","2do_saque_ganado_pct_2026"),("Service Games Won %","juegos_saque_g_pct_2026"),
                        ("BP Saved %","bp_salvados_pct_2026")]:
        val = player.get(col)
        if pd.notna(val):
            st.markdown(f'<div class="stat-row"><span class="stat-label">{label}</span><span class="stat-value">{val:.1f}%</span></div>', unsafe_allow_html=True)
...
presion_validos = {}
for k, (c_pct, c_rec) in presion.items():
    val = player.get(c_pct)
    if pd.notna(val) and val <= 1:
        presion_validos[k] = (val, player.get(c_rec, ""))
...
for label, c25, c26 in [
    ("1st Serve Won %","1er_saque_ganado_pct_2025","1er_saque_ganado_pct_2026"),
